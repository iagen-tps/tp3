...
La pantalla tiene tres columnas: a la izquierda el **listado de chats** (con botón
Nuevo), al centro el **selector de modelo**, el **contexto estático**, el hilo y el
composer (con el **effort** a la derecha del input), y a la derecha el **detalle** con
métricas acumuladas de la conversación.
...
perillas que ese modelo soporta: presupuesto de `thinking` en los slots 2 y 3;
`JSON Schema` en todos. El `effort` (slots 1, 3 y 4) vive en un select junto al input.
Cambiar de modelo pide confirmación y **abre una conversación nueva**: el chat anterior
queda en `chats/` con su `log.md` y `meta.json`.
...
**3 · Chatear.** Enter envía, Shift+Enter salta línea. La respuesta llega en streaming
(SSE). Bajo cada respuesta queda la lectura del turno: una barra a escala con la entrada
que vino del cache (verde), la que se reprocesó (gris) y la salida (violeta), y debajo
los números — entrada, cacheados, salida, razonamiento, costo y `cache_discount`. El
rail derecho lleva el acumulado y el **contador de prompts**, que es el número con el
que se audita el ejercicio 2.

**4 · Leer el log.** Cada chat vive en `chats/<stamp>__slotN__modelo/` con `log.md` y
`meta.json`. El log se crea con el primer mensaje y se reescribe entero después de cada
turno; el path aparece en el panel de detalle.
...
- `chats/` — conversaciones con `log.md` + `meta.json` (evidencia de los ejercicios 2 y 3)
